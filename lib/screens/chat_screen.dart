...
class ChatScreen extends StatefulWidget {
  ...
  const ChatScreen({super.key, required this.workspace, required this.channel});
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<String> _getSenderDisplayName(
    String senderId,
    String currentName,
  ) async {
            ...
            children: [
              ...
              if (_replyingTo != null && _editingMessage == null)
                FutureBuilder<String>(
                  future: _getSenderDisplayName(
                    _replyingTo!.senderId,
                    _replyingTo!.senderName,
                  ),
                  builder: (context, snapshot) {
                    final displayName =
                        snapshot.data ?? _replyingTo!.senderName;
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  try {
                    ...
                    _logger.logUI(
                      'ChatScreen',
                      isStarred ? 'message_unstarred' : 'message_starred',
                      data: {'messageId': message.id},
                    );
                  } catch (e) {
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            'Failed to ${isStarred ? 'unsave' : 'save'} message: $e',
                          ),
                          backgroundColor: Colors.red,
                        ),
                      );
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  try {
                    ...
                    _logger.logUI(
                      'ChatScreen',
                      message.isPinned ? 'message_unpinned' : 'message_pinned',
                      data: {'messageId': message.id},
                    );
                  } catch (e) {
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            'Failed to ${message.isPinned ? 'unpin' : 'pin'} message: $e',
                          ),
                          backgroundColor: Colors.red,
                        ),
                      );
